...
def expmap_so3(u):
    ...
    Arg: 
        u: (3, ) or (3, 1)
        Where u = omega * theta where omega is the UNIT AXIS of rotation and theta is the magnitude of rotation.
    ...
    return np.eye(3) + np.sin(u_norm) / u_norm * u_hat + (1 - np.cos(u_norm)) / (u_norm**2) * u_hat @ u_hat
...
def expmap_se3(xi, theta):
    ...
    if w_norm_sq < 1e-12:        # prismatic
        ...
        d = ((np.eye(3) - R) @ w_hat +
             (w_hat @ w_hat + np.eye(3)) * theta) @ v
        
        # d = ((np.eye(3) - R) @ w_hat / w_norm_sq +
        #      (w_hat @ w_hat / w_norm_sq + np.eye(3)) * theta) @ v # Case where w is not a unit vector. Not sure about correctness or usefulness.
...
def dualquatconj(Q):
    ...
    Return:
        ...
        Where Q* = qr* + ε qd*
    '''
    qr0, qr1, qr2, qr3, qd0, qd1, qd2, qd3 = Q
    return np.array([qr0, -qr1, -qr2, -qr3,
                     qd0, -qd1, -qd2, -qd3])

def dualquatsandwich(Q, xi_dq):
    '''
    Args:
        Q:     (8, )
        xi_dq: (8, )
    ...
    '''
    # qr0, qr1, qr2, qr3, qd0, qd1, qd2, qd3 = Q
    # Q_conj = np.array([qr0, -qr1, -qr2, -qr3,
    #                  qd0, -qd1, -qd2, -qd3])

    return dualquatmult(dualquatmult(Q, xi_dq), dualquatconj(Q)) 
...
def expmap_twistDQ(xi_dq, theta):
    """
    Dual‑quaternion exponential map for a twist 

    Parameters
    ----------
    xi_dq: (8,) ndarray
            [0, ω_x , ω_y , ω_z,  0, v_x , v_y , v_z ]
    theta : float
            Joint displacement (rad for revolute, meters for prismatic).
    ...
    if w_norm < 1e-12:     # prismatic
        qr = np.array([1, 0, 0, 0])
        qd = np.hstack([0, v * theta / 2])
    else:     # revolute
        qr = np.hstack([np.cos(theta / 2), w * np.sin(theta/2)])       # (4, )
        qd = np.hstack([0, np.sin(theta / 2) * v])                     # (4, )
...
def dualquat_to_SE3(Q):
    ...
    Arg:
        Q:  (8,), where Q[:4] is the real quaternion (rotation)
    ...
    d_quat = 2 * quatmult(qd, qr_conj)
    d = d_quat[1:]  # drop scalar part
...
def SE3_to_dualquat(g):
    '''
    Convert a SE(3) transformation matrix to a dual quaternion.
